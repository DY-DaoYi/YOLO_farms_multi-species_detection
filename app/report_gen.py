...
def generate_pdf_report(summary_data, detail_data, output_path):
    ...
    # 1. 标题页
    elements.append(Paragraph("农场多物种检测报告", title_style))
    ...
    total_label = summary_data.get('total_label', '检测目标总数')
    summary_table_data = [
    ...
    if total_class_counts:
        elements.append(Paragraph("二、各类别检测统计", header_style))
        class_stats_data = [["动物类别", "检测数量"]]
    ...
    for item in detail_data:
        ...
        if 'class_counts' in item and item['class_counts']:
            class_detail = ", ".join([f"{cls}: {cnt}只" for cls, cnt in item['class_counts'].items()])
