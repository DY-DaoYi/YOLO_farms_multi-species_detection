...
with tab2:
    ...
    if 'results_data' in st.session_state and st.session_state['results_data']:
        ...
        if total_class_counts:
            st.subheader("🐾 各类别检测统计")
            class_stats_df = pd.DataFrame([
                {"动物类别": cls, "检测数量": cnt} for cls, cnt in sorted(total_class_counts.items(), key=lambda x: x[1], reverse=True)
            ])
...
with tab4:
    ...
    本项目是基于 **YOLO (You Only Look Once)** 系列模型的农场多物种检测系统，专为识别农场环境中的多种动物而设计。
    
    ### ✨ 主要功能
    - **多模型兼容**: 无缝支持 YOLOv8, YOLO11, 以及最新的 YOLO26 模型。
    - **高精度实时检测**: 针对10种农场动物（猫、鸡、牛、狗、狐狸、山羊、马、人、浣熊、臭鼬）进行了深度优化和专门训练，支持图片和视频的快速检测。
    - **批量处理与报告生成**: 支持一次性导入多张图片进行批量检测，并能一键生成详细的 PDF 统计报告，方便农场管理人员存档和分析。
