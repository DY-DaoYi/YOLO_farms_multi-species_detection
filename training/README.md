...
```bash
# 从上次中断的地方继续训练
python train.py --model runs/train/animals/yolov8n_animals/weights/last.pt --resume
```
...
每个模型的训练结果保存在 `runs/train/{data_name}/{model_name}_animals/` 目录下：
...
*   **Q: 训练结果保存在哪里？**
    *   A: 默认保存在 `runs/train/` 目录下。每个实验会根据模型名称生成一个子文件夹，例如 `runs/train/animals/yolov8n_animals`。

*   **Q: 如何查看训练过程的可视化图表？**
    *   A: Ultralytics 会在 `runs/train/xxx/` 目录下自动生成 `results.png` 等图表。
    *   A: 你也可以使用本项目提供的 Streamlit App 的"模型训练数据分析"功能，直接读取 `results.csv` 进行交互式分析。
...
本项目使用 Roboflow 开源数据集，包含 10 类动物：
