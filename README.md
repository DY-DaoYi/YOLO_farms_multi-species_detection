# 🌾 基于 YOLO 的农场多物种检测系统 (Farm Multi-Species Detection System)
...
本项目是一个功能完整的农场多物种智能检测系统。基于最新的 Ultralytics YOLO 系列模型构建，能够识别 **10种农场动物**（猫、鸡、牛、狗、狐狸、山羊、马、人、浣熊、臭鼬）。本项目提供了美观、易用的 Streamlit Web 交互界面，非常适合作为**本科毕业设计**的参考与展示。
...
*   **🇨🇳 全中文支持**: 从界面菜单到检测框标签（如"牛"、"鸡"、"狗"），全部采用中文显示，符合国内用户习惯。
...
为了让系统能够识别农场动物，你需要加载训练好的 YOLO 模型文件（`.pt`）。
...
2.  **放置模型**：将你训练得到的 `best.pt` 文件复制到 `models/` 文件夹中（建议重命名为清晰的名称，如 `yolo11m_animals.pt`）。
...
```text
YOLO_farms_multi-species_detection/
├── app/
...
2.  **训练规模**:
    ...
    *   **训练轮数**: 100 Epochs / 模型
3.  **实际耗时与成本**:
    *   *总耗时*: 3 小时 34 分 (约 3.57 小时)
    *   *云服务器费率*: ¥9.35 / 小时
    *   **总算力成本**: **3.57 小时 x ¥9.35 ≈ ¥33.38**
